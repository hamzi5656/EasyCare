...
class Sticks extends StatefulWidget {
  const Sticks({super.key});

  @override
  State<Sticks> createState() => _MobilityState();
}

class _MobilityState extends State<Sticks> {
  ...
  @override
   ...
   getMobilityProducts(){
      ...
      FirebaseFirestore.instance.collection("Product").where("category", isEqualTo: "walkingsticks").where("popular", isEqualTo: false).get().then((response) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Walking sticks",style: TextStyle(color: Colors.white),),
